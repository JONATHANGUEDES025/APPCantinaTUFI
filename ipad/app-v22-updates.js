(function applyCantinaTufiV22Updates() {
  const FREE_PAYMENT_V22 = "Gratuidade";

  function isFreeSaleV22(saleOrMethod) {
    const method = typeof saleOrMethod === "string" ? saleOrMethod : saleOrMethod?.paymentMethod;
    return normalizePaymentMethod(method) === FREE_PAYMENT_V22;
  }

  function paymentFilterOptionsV22() {
    return ["Dinheiro", "Pix", PIX_MAE_MAG, "Debito", "Credito", FREE_PAYMENT_V22, "Fiado em aberto"];
  }

  const normalizePaymentMethodBaseV22 = normalizePaymentMethod;
  normalizePaymentMethod = function normalizePaymentMethodV22(method) {
    const key = debtorKey(method);
    if (key.includes("gratuidade") || key.includes("gratis") || key.includes("cortesia")) return FREE_PAYMENT_V22;
    return normalizePaymentMethodBaseV22(method);
  };

  const paymentReportKeyBaseV22 = paymentReportKey;
  paymentReportKey = function paymentReportKeyV22(sale) {
    if (sale.paymentMethod === "Fiado") return paymentReportKeyBaseV22(sale);
    return normalizePaymentMethod(sale.paymentMethod) || "-";
  };

  function addFreePaymentButtonV22() {
    const methods = document.querySelector(".payment-methods");
    if (!methods || methods.querySelector(`[data-method="${FREE_PAYMENT_V22}"]`)) return;
    const button = document.createElement("button");
    button.className = "pay-button";
    button.dataset.method = FREE_PAYMENT_V22;
    button.type = "button";
    button.textContent = FREE_PAYMENT_V22;
    button.addEventListener("click", () => setPayment(FREE_PAYMENT_V22));
  ...
  const renderSaleBaseV22 = renderSale;
  renderSale = async function renderSaleV22() {
    await renderSaleBaseV22();
    addFreePaymentButtonV22();
    updateCheckout();
  };

  updateCheckout = function updateCheckoutV22() {
    ...
    const freeSale = isFreeSaleV22(payment);
    totals.innerHTML = `
  ...
  confirmSale = async function confirmSaleV22() {
    try {
      ...
      const financialTotal = isFreeSaleV22(paymentMethod) ? 0 : calc.total;
  ...
  renderSales = async function renderSalesV22() {
    ...
    main.innerHTML = title("Historico", "Vendas salvas neste aparelho.", "") + `
      <div class="card">
        <div class="toolbar" style="margin-bottom:12px">
          ...
          <select id="salePayment"><option value="all">Todos pagamentos</option>${paymentFilterOptionsV22().map(method => `<option value="${esc(method)}">${esc(method)}</option>`).join("")}</select>
    ...
    ["saleFilter", "saleFrom", "saleTo", "salePayment", "saleStatus"].forEach(id => document.getElementById(id).addEventListener("input", () => renderSalesTable(sales)));
    renderSalesTable(sales);
  };

  renderSalesTable = function renderSalesTableV22(sales) {
    ...
    const rows = sales.filter(sale => {
      ...
      return searchMatch && paymentMatch && (!from || date >= from) && (!to || date <= to) && (status === "all" || sale.status === status);
    }).map(sale => `
      <tr>
        <td>#${sale.id}</td>
  ...
  renderStock = async function renderStockV22() {
  ...
  renderReports = async function renderReportsV22() {
    main.innerHTML = title("Resumo", "Faturamento, lucro e produtos vendidos.", "") + `
      <div class="toolbar" style="margin-bottom:14px">
        ...
        <select id="reportPayment"><option value="all">Todos pagamentos</option>${paymentFilterOptionsV22().map(method => `<option value="${esc(method)}">${esc(method)}</option>`).join("")}</select>
      </div>
  ...
  loadReport = async function loadReportV22() {
    ...
    const sales = (await all("sales")).filter(sale => {
      const date = sale.date.slice(0, 10);
      const paymentMatch = payment === "all" || paymentReportKey(sale) === payment;
      return sale.status === "ativa" && paymentMatch && (!from || date >= from) && (!to || date <= to);
    });
    const total = sales.reduce((sum, sale) => sum + sale.total, 0);
    const profit = sales.reduce((sum, sale) => sum + sale.items.reduce((itemSum, item) => {
      const unitRevenue = isFreeSaleV22(sale) ? 0 : item.unitPrice;
      return itemSum + ((unitRevenue - item.productCost) * item.quantity);
    }, 0), 0);
    const payments = {};
    const products = {};
    const paymentOrder = paymentFilterOptionsV22();
    sales.forEach(sale => {
      ...
      sale.items.forEach(item => {
        ...
        products[item.productName].total += isFreeSaleV22(sale) ? 0 : item.subtotal;
      });
    });
