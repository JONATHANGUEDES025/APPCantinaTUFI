(async function loadCantinaTufiIpad() {
  ...
  try {
    ...
    applyCantinaTufiFinalAdjustments();
  } catch (error) {
    console.error(error);
  ...
  function applyCantinaTufiFinalAdjustments() {
    ...
    const HOUSE_DEBTOR_SEEDS = [
      ...
      "Maria Clara", "Maria Flor", "Mônica/Alexandre", "Monica Mello", "Pamela", "Paula", "Rosangela", "Vica", "Wellington", "Yasmin"
    ...
    function patchPayment() {
      ...
      loadReport = async function patchedLoadReport() {
        ...
        const paymentRows = paymentOrder.map(method => payments[method] || { method, count: 0, total: 0 }).concat(Object.values(payments).filter(row => !paymentOrder.includes(row.method)).sort((a, b) => b.total - a.total));
    ...
    function patchFiados() {
      const oldGroupDetails = fiadoGroupDetails;
      fiadoGroupDetails = function patchedFiadoGroupDetails(index) {
        oldGroupDetails(index);
        const selected = document.getElementById("selectedSettleMethod");
        if (selected) selected.innerHTML = SETTLEMENT_METHODS.map(method => `<option>${esc(method)}</option>`).join("");
      };
      const oldSettleGroup = settleFiadoGroup;
      settleFiadoGroup = function patchedSettleFiadoGroup(index) {
        oldSettleGroup(index);
        const selected = document.getElementById("settleMethod");
        if (selected) selected.innerHTML = SETTLEMENT_METHODS.map(method => `<option>${esc(method)}</option>`).join("");
      };
      const oldSettleSelected = settleSelectedFiados;
      settleSelectedFiados = async function patchedSettleSelectedFiados(index) {
        const selected = document.getElementById("selectedSettleMethod");
        if (selected) selected.value = normalizePayment(selected.value);
        return oldSettleSelected(index);
      };
      const oldSaveSettlement = saveSettlement;
      saveSettlement = async function patchedSaveSettlement(index) {
        const selected = document.getElementById("settleMethod");
        if (selected) selected.value = normalizePayment(selected.value);
        return oldSaveSettlement(index);
      };
    }
      ...
      renderSystem = async function renderSystem() {
        ...
        main.innerHTML = title("Ajustes", "Backup, dados e limpeza do aplicativo.", "") + `
          <section class="grid two">
            <div class="card"><h2>Dados neste aparelho</h2><p>Produtos: <b>${products.length}</b><br>Vendas: <b>${sales.length}</b><br>Devedores da TUFI: <b>${houseDebtors.length}</b><br>Movimentos de estoque: <b>${stock.length}</b></p></div>
            <div class="card"><h2>Backup</h2><p class="muted">Como os dados ficam neste iPad, exporte backup com frequência.</p><div class="backup-actions"><button class="primary" onclick="exportBackup()">Exportar backup</button><label class="secondary file-input">Importar backup<input type="file" accept="application/json" onchange="importBackupFile(event)"></label></div></div>
          </section>
          <section class="grid two" style="margin-top:14px"><div class="card"><h2>Zerar sistema</h2><p class="muted">Use antes de entregar o app limpo. Exporte um backup antes de apagar.</p><button class="danger" onclick="resetSystem()">Zerar tudo neste iPad</button></div></section>`;
      };
    }
    ...
    function patchReset() {
      resetSystem = async function resetSystem() {
        ...
        state.cart = [];
        clearSaleDraft();
        window.__cantinaTufiFinalSeedDone = false;
